...
function save_additional(){
	...
	$ket="Mutasi Bank HO - Wilayah";			
	
	//POSTING
	$account_masuk 				= get_coa_bank($fk_bank_masuk,$fk_cabang_masuk);
	$account_keluar 			= get_coa_bank($fk_bank_keluar,$fk_cabang_keluar);

	$piutang					= get_coa_cabang($fk_cabang_masuk);	
		
	$arrPost = array();
	$arrPost["piutang"]				= array('type'=>'d','value'=>$total,'reference'=>$keterangan,'account'=>$piutang);
	...
	$arrPost["hutang_afiliasi_ho"]	= array('type'=>'c','value'=>$total,'reference'=>$keterangan);	
	//cek_balance_array_post($arrPost);	
