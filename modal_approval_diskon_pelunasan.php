...
function fGetSbgData(){
	...
	lSentText="table=(select fk_sbg,nm_customer,diskon,fk_cif from tblinventory left join (select fk_sbg as fk_sbg1, diskon from data_fa.tbldiskon_pelunasan) as tblmain on fk_sbg=fk_sbg1 left join tblcustomer on no_cif=fk_cif)as tbl&field=(nm_customer||'¿'||fk_cif||'¿'||diskon)&key=fk_sbg&value="+document.form1.fk_sbg.value
...
<!-- content begin -->
			<table cellpadding="0" cellspacing="1" border="0" width="100%">
                        ...
                        <tr bgcolor="efefef">
                            ...
                            <td style="padding:0 5 0 5" width="20%">Diskon Pelunasan</td>
...
function save_data(){
	...
	if(!pg_query("insert into data_fa.tbldiskon_pelunasan_log select *,'".$_SESSION["id"]."','".$_SESSION["username"]."','#".date("Y/m/d H:i:s")."#','UB' from data_fa.tbldiskon_pelunasan where fk_sbg='".$fk_sbg."'")) $l_success=0;
		
	if(!pg_query("
	update data_fa.tbldiskon_pelunasan set 
	is_approval='t'
	where fk_sbg='".$fk_sbg."'
	")) $l_success=0;

	if(!pg_query("insert into data_fa.tbldiskon_pelunasan_log select *,'".$_SESSION["id"]."','".$_SESSION["username"]."','#".date("Y/m/d H:i:s")."#','UA' from data_fa.tbldiskon_pelunasan where fk_sbg='".$fk_sbg."'")) $l_success=0;	
	
	
	//$l_success=0;
	if ($l_success==1){
		$strmsg="Data Diskon Pelunasan Berhasil ter-Approve.<br>";
		$j_action="lInputClose=getObjInputClose();lInputClose.close();";
		pg_query("COMMIT");
	}else{
		$strmsg="Error :<br>Data Diskon Gagal Approve.<br>";
		pg_query("ROLLBACK");
	}
