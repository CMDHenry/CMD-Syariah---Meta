	...
	<tr>
		<td class="border">
			<table cellpadding="0" cellspacing="0" border="0" width="100%" bgcolor="#D0E4FF">
				<td align="center" class="judul_menu">UPLOAD METODE PERHITUNGAN(.xls)</td>
			</table>		
		</td>
	</tr>
    <tr>
...
function update_database($file){
	...
	for($i=2;$i<=$total_data;$i++){
		$lrow=array(
				...
				'fk_metode'=>$array_excel->val($i,'B'),
			);
			
		//echo 'bb'.$lrow["fk_sbg"].'aaa';
		$lrow["fk_sbg"]=preg_replace('/[^A-Za-z0-9-.; \-]/', '', $lrow["fk_sbg"]);
		//echo 'cc'.$lrow["fk_sbg"].'ddd';
		//showquery("select * from tblmetode_perhitungan_jaminan where kd_metode='".$lrow["fk_metode"]."' ");
		if(!pg_num_rows(pg_query("select * from tblmetode_perhitungan_jaminan where kd_metode='".$lrow["fk_metode"]."' "))){
			echo "Metode baris ".($i-1)." tidak ada <br>";
			$l_success=0;
		}	
		
		//log begin
		if(!pg_query("insert into data_gadai.tblproduk_cicilan_log select *,'".$_SESSION["id"]."','".$_SESSION["username"]."','#".date("Y/m/d H:i:s")."#','UB' from data_gadai.tblproduk_cicilan where no_sbg='".$lrow["fk_sbg"]."'")) $l_success=0;
		//end log
	
		if(!pg_query("update data_gadai.tblproduk_cicilan set fk_metode=".(($lrow["fk_metode"]=="")?"null":"'".convert_sql($lrow["fk_metode"])."'")." where no_sbg='".$lrow["fk_sbg"]."'")) $l_success=0;
		//showquery("update data_gadai.tblproduk_cicilan set fk_metode=".(($lrow["fk_metode"]=="")?"null":"'".convert_sql($lrow["fk_metode"])."'")." where no_sbg='".$lrow["fk_sbg"]."'");
...
function create_excel(){
	...
	$xls->xlsWriteLabel(0,1,"Kode Metode");	
	
	//===============================================
	
	$xls->xlsOutput($upload_path,"temp");
	echo "Klik untuk men-download<br>";
	echo "<a href='file/temp/metode.xls'>METODE</a><br>";
}
?>
