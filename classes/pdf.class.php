...
EOT;
    ...
    function o_info($id,$action,$options=''){
        ...
        case 'new':
            $this->infoObject=$id;
            $date='D:'.date('Ymd'); //R and OS php pdf writer, http://www.ros.co.nz
            $this->objects[$id]=array('t'=>'info','info'=>array('Creator'=>'www.meta-advance.com','CreationDate'=>$date));
            break;
        case 'Title':
        case 'Author':
        case 'Subject':
    ...
    function openFont($font) {
        ...
        if (!isset($this->fonts[$font]) && file_exists($dir.$metrics_name)) {
			...
			//echo $this->tempPath.'==='.$cachedFile.'<br>';
    ...
    {
		//tambahan cek file exists klo gak ada default nya helvetica
		if(!file_exists($fontName.'.afm')){
			$fontName = './fonts/Helvetica';
		}
		//==================================
    ...
    function addTextWrap($x, $y, $width, $size, $text, $justification = 'left', $angle = 0, $test = 0){
        ...
        $this->setCurrentFont();
        if (!$test){
			//echo '=='.$x.'-'.$y.'-'.$size.'-'.$text.'-'.$angle.'-'.$adjust.'-'.$angle.'===';
