...
//include_once '../requires/file.inc.php';
class Cezpdf extends Cpdf {
	...
	global $path_site;
        if (!is_array($paper)){ //2 width, 3 height
            switch (strtoupper($paper)){
				case 'A4.5': {$size = array(0,0,595,401); break;} //array(0,0,595,421); break;}
				...
				case 'F4': {$size = array(0,0,595.28,935.43); break;}
				case 'KWITANSI': {$size = array(0,0,705,301); break;}
				case 'F4/3': {$size = array(0,0,595.28,450); break;}
		...
		if($_SERVER['HTTP_HOST']=="meta-server"){
			$path_site="D:/Development/Web Project/capella/website/";
		}
		
		if($_SERVER['HTTP_HOST']=="116.90.163.21"||$_SERVER['HTTP_HOST']==""||$_SERVER['HTTP_HOST']=="cmds.capellamultidana.com"){
			$path_site="/www/erp/website/";
		}
		$this->tempPath = $path_site.'print'; // klo di cloud live > '/temp';
		$this->selectFont('./fonts/Courier'); //setting jenis font di sini //kunci dg font yg paling ok
		//end setting tambahan
    ...
    function ezTableColumnHeadings($cols,$pos,$maxWidth,$height,$decender,$gap,$size,&$y,$optionsAll=array()){
        ...
        while ($ok==0){
            foreach ($cols as $colName=>$colHeading){
                $this->ezSetY($y); //aslinya justification doank, ini diubah jadi heading_justification
                if (isset($options[$colName]) && isset($options[$colName]['heading_justification'])){
                    $justification = $options[$colName]['heading_justification'];
                } else {
                    $justification = 'center'; //aslinya left
                }
     ...
     * @return unknown_type
     */
    function ezTable(&$data,$cols='',$title='',$options='',$size=''){ //tambah size utk akomodir param size dr versi lama
		...
		//add on tambahan utk convert size dari versi lama
		if(is_array($size)){
			foreach($size as $l_key => $l_width){
				$options['cols'][$l_key]['width'] = $l_width;
			}
		}
		//==========================================
    ...
    function ezText($text,$size=0,$options=array(),$test=0){
		...
		//$this->selectFont('fonts/Times-Bold');
    ...
    function ezImage($image, $pad = 5, $width = 0, $resize = 'full', $just = 'center', $border = '') {
        ...
        if ($width < $bigwidth) {
            ...
            if ($just == 'left') {
			 ...
			 if ($just == 'left_barcode') {
                $offset = -45;
            }
			 if ($just == 'right_barcode') {
                $offset = 225;
            }
        }
