	...
	function _fTableCekAjaxState(){
		...
		if (this.readyState == 4){
			if (this.status==200 && this.responseText!="") {
            	...
            	lArrTargetReferer=this.targetReferer.split(",")               
	            for (i=0;i<lArrTargetReferer.length;i++){
					if (this.targetReferer) this.targetCell.parentNode.cells[lArrTargetReferer[i]].innerHTML=""
                }
				this.targetCell.errorAjax=true;
...
//	}
	...
	function _fTableCreateField(pCell,pColumn,pValue){
		if(document.form1.status_fatg.value=='Perpanjangan'){
        	if(pCell.cellIndex==0 || pCell.cellIndex==1 || pCell.cellIndex==2){
	            pColumn.type='readonly'
            }
        }
...
//			lCell.setAttribute("align","center")
	...
	function _fTableWriteValue(pCell,pCellData){
		...
		if (lArrTable[pCell.cellIndex].type=="readonly"){
             if (pCell.cellIndex==8) {
				lRow=pCell.parentNode
                if(lRow.cells[7].innerHTML=='Antam'){
                	harga_antam=document.form1.harga_stle_antam.value
                }else if(lRow.cells[7].innerHTML=='Non Antam'){
                 	harga_antam=document.form1.harga_stle_non_antam.value
                }else if(lRow.cells[7].innerHTML=='Perhiasan'){
                 	harga_antam=document.form1.harga_stle_perhiasan.value
                }
                else if(lRow.cells[7].innerHTML=='Berlian'){
                 	harga_stp=document.form1.harga_stp.value
                }
                if(lRow.cells[7].innerHTML=='Antam'||lRow.cells[7].innerHTML=='Non Antam'||lRow.cells[7].innerHTML=='Perhiasan'){
                var harga =  parseFloat( lRow.cells[4].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) * parseFloat(lRow.cells[6].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) * harga_antam / 24
                } else {
                var harga =  parseFloat( lRow.cells[2].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) * parseFloat(lRow.cells[4].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) * harga_stp
                }
              	lValue=harga;  
                pCell.innerHTML=number_format(lValue,false)
                pCell.align="right"
                grandTotal(pCell)
             }             
			else{
        ...
        if(pCell.cellIndex=="8")grandTotal(pCell)
	}
...
//			}
    ...
    function grandTotal(pObj,pParam){
        ...
        for (x=2;(x<lTable.rows.length-1);x++){
            total += parseFloat(lTable.rows[x].cells[8].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",","."))
            jumlah_barang += parseFloat(lTable.rows[x].cells[2].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",","."))
            //jumlah_barang += 1
        }
        ...
        document.getElementById("divjumlah_barang").innerHTML=jumlah_barang
        document.form1.jumlah_barang.value=jumlah_barang
        
        fGetPenaksirData()
    }
    
    function fCalc(pTable){
//    	
    }
