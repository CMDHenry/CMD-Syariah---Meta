    	...
    	if (pColumn.is_readonly!='t') {
            switch (pColumn.type) {
                ...
                case "get":
                    ...
                    lObjImg.setAttribute("onclick", "fGetNC(false, '"+pColumn.source+"', 'kd_"+pColumn.source+"' ,'', _fTableGetInput(this), _fTableGetNext(this),'','','','"+pColumn.id_detail_field+"');")
...
//				else pCell.innerHTML=pCellData.innerHTML
			...
			if (lInput) {                
				switch (lArrTable[pCellData.cellIndex].type) {
                	...
                	case "numeric":
                        ...
                        break;
					case "list_manual":
                    	if(lInput.options[lInput.selectedIndex].value=='-- Pilih --')pCell.innerHTML=""
						else pCell.innerHTML=lInput.options[lInput.selectedIndex].value
						lInput.selectedIndex=0
						break;
					case "checkbox":
					case "radio":
						...
						if (pCell!=pCellData){
							...
							pCell.children[0].checked=lInput
        ...
        // 
        if(pCell.cellIndex=="3" && lTable.id == "Table_payment_request")grandTotal(pCell)
        //confirm(pCell.cellIndex)
        if(pCell.cellIndex=="1" && lTable.id != "Table_payment_request")grandTotal2(pCell)
...
//			}
				...
				for (lIndexRow=2;lIndexRow<this.table.rows.length-1;lIndexRow++){
					...
					for (lIndexCell=0;lIndexCell<this.table.rows[lIndexRow].cells.length-1;lIndexCell++){
						if (this.columns[lIndexCell].type=="checkbox" || this.columns[lIndexCell].type=="radio" ){
                        	...
                        	if(this.table.rows[lIndexRow].cells[lIndexCell].innerHTML=='-- Pilih --')lStrRow+=""+"»"
							else lStrRow+=this.table.rows[lIndexRow].cells[lIndexCell].innerHTML+"»"
						}
					}
			...
			for (lIndex=0;lIndex<(lRowData.length-1);lIndex++){
				...
				for (var lIndex2=0;lIndex2<lCellData.length;lIndex2++){
					...
					if (this.columns[lIndex2].type=="checkbox") {
                    	if(lCellData[lIndex2]=='t')lChecked='checked'
                        else lChecked=''
                        //"+lCellData[lIndex2]+" disabled='disabled'
						lCell.innerHTML="<input type='checkbox' name='"+this.columns[lIndex2].name+"' disabled='disabled' "+lChecked+" >"
						lCell.align="center"
    ...
    function grandTotal(pObj,pParam){
        ...
        for (x=2;(x<lTable.rows.length-1);x++){ 
        	//confirm(lTable.rows[x].cells[4].children[0].checked)           
        	if (lTable.rows[x].cells[1].innerHTML.indexOf("212100000")!=-1)(
            	total_pajak += parseFloat(lTable.rows[x].cells[3].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",","."))
            )
            else (
            	total += parseFloat(lTable.rows[x].cells[3].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",","."))
            )
        }       
        if(parseFloat(total) < 0)flag = true
        else flag = false
        
        document.getElementById("divtotal").innerHTML=number_format(total,true,flag)
        document.form1.total.value=total
        
        
        
        //confirm(document.form1.total);
        
       // document.getElementById("divtotal_detail").innerHTML=number_format(total,true,flag)
       // document.form1.total_detail.value=total
        
        total_setelah_pajak =total - total_pajak
        //document.getElementById("divtotal_setelah_pajak").innerHTML=number_format(total_setelah_pajak,true)
        //document.form1.total_setelah_pajak.value=total_setelah_pajak
        
       // total_cabang=document.form1.total_cabang.value;
        
      //  total_all=total*total_cabang
        //document.getElementById("divtotal_all").innerHTML=number_format(total_all,true,flag)
        //document.form1.total_all.value=total_all

       // total_all_setelah_pajak=total_setelah_pajak*total_cabang
        //document.getElementById("divtotal_all_setelah_pajak").innerHTML=number_format(total_all_setelah_pajak,true)
        //document.form1.total_all_setelah_pajak.value=total_all_setelah_pajak

    }
    function grandTotal2(pObj,pParam){   		
    	if(pParam==null || pParam==undefined)lTable=pObj.parentNode.parentNode
        else if(pParam=='1')lTable=pObj
		total_cabang=0
        for (x=2;(x<lTable.rows.length-1);x++){ 
        ...
        if(parseFloat(total_cabang) < 0)flag = true
        else flag = false
        //confirm(document.form1.total);
        //document.getElementById("divtotal_cabang").innerHTML=(total_cabang)
        //document.form1.total_cabang.value=total_cabang

		//total=document.form1.total_detail.value;      
        total_all=total*total_cabang     
       //document.getElementById("divtotal_all").innerHTML=number_format(total_all,true,flag)
        //document.form1.total_all.value=total_all
        
		//total_setelah_pajak=document.form1.total_setelah_pajak.value;
        //total_all_setelah_pajak=total_setelah_pajak*total_cabang
        //document.getElementById("divtotal_all_setelah_pajak").innerHTML=number_format(total_all_setelah_pajak,true)
        //document.form1.total_all_setelah_pajak.value=total_all_setelah_pajak
    }
