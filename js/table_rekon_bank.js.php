...
//			}
    ...
    function grandTotal(pObj,pParam){
        ...
        for (x=2;(x<lTable.rows.length-1);x++){
         	if(lTable.rows[x].cells[4].innerHTML=="Masuk"){
        		total+=parseFloat(lTable.rows[x].cells[3].innerHTML.replace(_RegExpEraseNumFormat,""))
        	}
            if(lTable.rows[x].cells[4].innerHTML=="Keluar"){
        		total-=parseFloat(lTable.rows[x].cells[3].innerHTML.replace(_RegExpEraseNumFormat,""))
        	}
        }
        ...
        //sisa_saldo = document.form1.saldo_bank.value - document.form1.total.value
        //document.getElementById("divsisa_saldo").innerHTML=number_format(sisa_saldo,true)
        //document.form1.sisa_saldo.value=sisa_saldo
