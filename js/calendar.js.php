...
if(typeof(offX) == "undefined")
	offX = -120 // x distance from the mouse.
...
function cel(obj){ return document.createElement(obj);  }
function sa(obj, atname, atprop){ return  obj.setAttribute(atname, atprop);  }
function appendc(obj, elem){ return  obj.appendChild(elem); }
function cNode(obj, txt){ return obj.appendChild(document.createTextNode(txt)); }
...
var _fCalendarFunction

function fPopCalendar(pObj,pFunction)
{
	if (pFunction) _fCalendarFunction=pFunction
	else _fCalendarFunction=null
    
	
	tmpString = new String();
	lObj = pObj;
	tmpString = lObj.value;
	...
	if(folowMouse)
	{
		...
		{
        	//kalau misalnya udah mentok dibawah
        	moveHeight = 0 
			if(parseInt(tempY)+parseInt(offY)+165 >= window.innerHeight) moveHeight = 200
            //end===============================
        	document.getElementById('calendar').style.left = parseInt(tempX)+parseInt(offX)+ 'px';
		    document.getElementById('calendar').style.top = ( parseInt(tempY)+parseInt(offY)-parseInt(moveHeight) )+ 'px';
		}
	}
...
function fillBackDate(tDay, tMonth, tYear)
{
	...
	closeCalendar();
	if (_fCalendarFunction) _fCalendarFunction(lObj,vali);
}
