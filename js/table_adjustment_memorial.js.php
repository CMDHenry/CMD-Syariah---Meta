    	...
    	if (pColumn.is_readonly!='t') {
            switch (pColumn.type) {
                ...
                case "radio":
                    ...
                    break;
                case "get":
                	//confirm(pColumn.id_detail_field.value)
                    pCell.innerHTML=""
...
//				else pCell.innerHTML=pCellData.innerHTML
        ...
        if(pCell.cellIndex=="4")grandTotal(pCell)
	}
...
//			}
	...
	function table (){
				...
				for (lIndexRow=2;lIndexRow<this.table.rows.length-1;lIndexRow++){
					...
					for (lIndexCell=0;lIndexCell<this.table.rows[lIndexRow].cells.length-1;lIndexCell++){
						if (this.columns[lIndexCell].type=="checkbox" || this.columns[lIndexCell].type=="radio" ){
							...
							lStrRow+=((lInput.checked)?'t':'f')+"<?=chr(187)?>"
                            ...
                            if(ltmp.match(/^\d{1,2}\/\d{1,2}\/\d{4}$/)){
                                ...
                                lStrRow+=lDate+"<?=chr(187)?>"
                            }else{
                            	lStrRow+="<?=chr(187)?>"
                            }
                        } else {
							lStrRow+=this.table.rows[lIndexRow].cells[lIndexCell].innerHTML+"<?=chr(187)?>"
						}
					}
					lStrIsi+=lStrRow.substring(0,lStrRow.length-1)+"<?=chr(191)?>"
				}
                //confirm(lStrIsi)
				return lStrIsi
			} else {
		...
		this.setIsi=function(pIsi){
			...
			for (lIndex=0;lIndex<(lRowData.length-1);lIndex++){
				...
				lRow.onmouseover=_fTableRowMouseOver
				lRow.setAttribute("bgcolor","#e0e0e0")
				lCellData=lRowData[lIndex].split("<?=chr(187)?>")
    ...
    function grandTotal(pObj,pParam){
        ...
        if(lTable.rows[x].cells[3].innerHTML=="C"){
        	total_credit+=parseFloat(lTable.rows[x].cells[4].innerHTML.replace(_RegExpEraseNumFormat,""))
        	} else {
        	total_debit+=parseFloat(lTable.rows[x].cells[4].innerHTML.replace(_RegExpEraseNumFormat,""))
           }
        }
       
        //confirm(document.form1.total);

       	document.getElementById("div_total_debit").innerHTML = number_format((total_debit),true)
        document.form1.total_debit.value = total_debit
        document.getElementById("div_total_credit").innerHTML = number_format((total_credit),true)
        document.form1.total_credit.value = total_credit
