...
?>
	...
	function _fTableCekAjax(pObj,pValue){
        ...
        lRow=pObj.parentNode.parentNode
		...
		if (lColumn.table_db!="" && lColumn.field_get!="") {
            ...
            //lObjLoad.open("POST","ajax/get_data_harga.php",true);
			lObjLoad.setRequestHeader("Content-Type","application/x-www-form-urlencoded")
			lObjLoad.setRequestHeader("Content-Length",lSentText.length)
			lObjLoad.setRequestHeader("Connection","close")
			lObjLoad.send(lSentText);
		}/*else if(lColumn.name=="tahun"){
			lSentText='fk_barang='+encodeURI(lRow.cells[0].children[0].value)+'&tahun='+pValue     
            lObjLoad = getHTTPObject()
			lObjLoad.onreadystatechange=_fTableCekAjaxState2
			lObjLoad.targetCell=lCell
			lObjLoad.targetReferer=lColumn.referer
			lObjLoad.open("POST","ajax/get_data_harga.php",true);
			lObjLoad.setRequestHeader("Content-Type","application/x-www-form-urlencoded")
         ...
         //confirm("aaa");
			if (this.status==200 && this.responseText!="") {
				if (this.responseText=="-") {
                    ...
                    lArrResponseText=this.responseText.split("|")
                    lArrTargetReferer=this.targetReferer.split(",")
                 	this.targetCell.parentNode.cells[6].innerHTML=number_format(lArrResponseText[0])   
                    this.targetCell.parentNode.cells[7].innerHTML=number_format(lArrResponseText[0])                    
					this.targetCell.errorAjax=false;				
                }
			} else {
            	lArrTargetReferer=this.targetReferer.split(",")               
    	...
    	if (pColumn.is_readonly!='t') {
            switch (pColumn.type) {
                ...
                case "text":
                    ...
                    lObj.setAttribute("maxlength",pColumn.maxlength)
                    lObj.setAttribute("value",pValue)
                    lObj.setAttribute("onchange","_fTableCekAjax(this,this.value);fNextFocus(event,_fTableGetNext(this))")
                    pCell.appendChild(lObj)
                    break;
                case "text_area":
                    pCell.innerHTML=""
                ...
                case "get":
                    ...
                    //lTable.id=='Table_detail_kondisi_barang'
                    if(lTable.id=="Table_taksir_umum"){
...
//			lCell.setAttribute("align","center")
	...
	function _fTableWriteValue(pCell,pCellData){
		...
		if (lArrTable[pCell.cellIndex].type=="readonly"){
	        if (pCell.cellIndex==6 && lTable.id=='Table_taksir_umum') {
            	var lValue =  parseFloat( lRow.cells[4].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",","."))-parseFloat( lRow.cells[5].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) 
        ...
        //if(pCell.cellIndex=="7")grandTotal(pCell)
        if(pCell.cellIndex=="1" && lTable.id=='Table_detail_kondisi_barang'){
        }
	}
...
//			}
	...
	function _fTableAddNew(){
       ...
       // fGetNilaiKondisiData(table_taksir_umum.getIsi(),table_detail_kondisi_barang.getIsi())
	}
	...
	function _fTableAddEdit(){
        ...
        //confirm(table_detail_kondisi_barang.getIsi())
        //confirm(table_taksir_umum.getIsi())
       // if(lTable.id=='Table_detail_kondisi_barang'){ 
			...
			for (lIndex=0;lIndex<(lRowData.length-1);lIndex++){
				...
				for (var lIndex2=0;lIndex2<lCellData.length;lIndex2++){
					...
					if (this.columns[lIndex2].type=="checkbox") {
	                    if(lCellData[lIndex2]=='t')lCellData[lIndex2]='checked'
