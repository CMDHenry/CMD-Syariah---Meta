...
//			lCell.setAttribute("align","center")
	...
	function _fTableWriteValue(pCell,pCellData){
		...
		if(lArrTable[pCell.cellIndex].is_numeric=="t")pCell.align="right"
        else pCell.align="left"
		eval("lArrTable=arrField"+lTable.id)       
		if (lArrTable[pCell.cellIndex].type=="readonly"){
			if (pCell.cellIndex==4) {
				lRow=pCell.parentNode
          
                var harga =  parseFloat( lRow.cells[3].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) * parseFloat(lRow.cells[2].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) / 100 / 360
              	lValue=harga;  
                pCell.innerHTML=number_format(lValue,false)
                pCell.align="right"
                //grandTotal(pCell)
                
             } else if (pCell.cellIndex==7) {
				lRow=pCell.parentNode
          		//confirm('aaaa');
                var harga =  parseFloat( lRow.cells[1].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) + parseFloat(lRow.cells[5].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) - parseFloat(lRow.cells[6].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",","."))
              	lValue=harga;  
                pCell.innerHTML=number_format(lValue,false)
                pCell.align="right"
                //grandTotal(pCell)
             } else if (pCell.cellIndex==8) {
				lRow=pCell.parentNode
          		//confirm('aaaa');
                var harga =  parseFloat( lRow.cells[2].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) - parseFloat(lRow.cells[5].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",",".")) + parseFloat(lRow.cells[6].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",","."))
              	lValue=harga;  
                pCell.innerHTML=number_format(lValue,false)
                pCell.align="right"
                //grandTotal(pCell)
             } else { if (pCell!=pCellData) {
			...
			if (lAlerttxt=="") {
				...
				for (lIndexRow=2;lIndexRow<this.table.rows.length-1;lIndexRow++){
					...
					for (lIndexCell=0;lIndexCell<this.table.rows[lIndexRow].cells.length-1;lIndexCell++){
                        ...
                        } else if(this.columns[lIndexCell].type=="numeric"||(this.columns[lIndexCell].is_numeric=="t" &&this.columns[lIndexCell].type=="readonly")) {
			...
			for (lIndex=0;lIndex<(lRowData.length-1);lIndex++){
				...
				for (var lIndex2=0;lIndex2<lCellData.length;lIndex2++){
					...
					if (this.columns[lIndex2].type=="checkbox") {
						...
						lCell.align="center"
					} else if (this.columns[lIndex2].type=="numeric" ||this.columns[lIndex2].is_numeric=="t") {
