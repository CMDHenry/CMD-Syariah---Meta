            ...
            switch (pColumn.type) {
                ...
                case "get":
                    ...
                    lObjImg.align="absmiddle"
                 	lObjImg.setAttribute("onclick", "fGetNC(false, '"+pColumn.source+"', 'jenis_mutasi='+escape(document.form1.jenis_mutasi.value)+'&fk_bank_masuk='+escape(document.form1.fk_bank_masuk.value)+'&fk_bank_keluar='+escape(document.form1.fk_bank_keluar.value)+'&fk_cabang_wilayah='+escape(document.form1.fk_cabang_ho.value)+'&kd_"+pColumn.source+"' ,'', _fTableGetInput(this), _fTableGetNext(this),'','','','"+pColumn.id_detail_field+"','mutasi_bank_wilayah_ke_cabang');")                                        
...
//				else pCell.innerHTML=pCellData.innerHTML
        ...
        if(pCell.cellIndex=="2")grandTotal(pCell)
	}
...
//			}
    ...
    function grandTotal(pObj,pParam){
        ...
        for (x=2;(x<lTable.rows.length-1);x++){
            total += parseFloat(lTable.rows[x].cells[2].innerHTML.replace(_RegExpEraseNumFormat,"").replace(",","."))
            jumlah_detail += 1
        }
        if(parseFloat(total) < 0)flag = true
        else flag = false
        //confirm(document.form1.total_detail);
        document.getElementById("divtotal_detail").innerHTML=number_format(total,true,flag)
        document.form1.total_detail.value=total
        
        document.getElementById("divjumlah_detail").innerHTML=number_format(jumlah_detail,true,flag)
        document.form1.jumlah_detail.value=jumlah_detail
    }
    function fCalc(){
