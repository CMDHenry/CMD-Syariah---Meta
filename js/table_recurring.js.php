...
//			}
    ...
    function grandTotal(pObj,pParam){
        ...
        if(lTable.rows[x].cells[2].innerHTML=="C"){
        	total_credit+=parseFloat(lTable.rows[x].cells[3].innerHTML.replace(_RegExpEraseNumFormat,""))
        	} else {
        	total_debit+=parseFloat(lTable.rows[x].cells[3].innerHTML.replace(_RegExpEraseNumFormat,""))
           }
        }
       
        //confirm(document.form1.total);

       	document.getElementById("divtotal_debit").innerHTML = number_format((total_debit),true)
        document.form1.total_debit.value = total_debit
        document.getElementById("divtotal_credit").innerHTML = number_format((total_credit),true)
