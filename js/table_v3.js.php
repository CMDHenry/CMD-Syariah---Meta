...
?>
	...
	function _fTableCekAjax(pObj,pValue){
		...
		if (lColumn.table_db!="" && lColumn.field_get!="") {
			lSentText='table='+lColumn.table_db+'&field='+lColumn.field_get+'&key='+lColumn.field_key+'&value='+pValue+'&is_date='+lColumn.is_date
		...
		if (this.readyState == 4){
			if (this.status==200 && this.responseText!="") {
				if (this.responseText=="-") {
					...
					if (this.targetReferer) this.targetCell.parentNode.cells[this.targetReferer].innerHTML=this.responseText
    	...
    	if (pColumn.is_readonly!='t') {
            switch (pColumn.type) {
                case "numeric":
                    pCell.innerHTML=""
                    pCell.align="right"
                    //knp kemarin di remark? sapa yg remark nih?
                    lObjHidden=document.createElement("input");
                    lObjHidden.setAttribute("type","hidden")
                    lObjHidden.setAttribute("value",pValue.replace(_RegExpEraseNumFormat,"").replace(",","."))
                ...
                case "get":
                    ...
                    lObjImg.setAttribute("onclick", "fGetNC(false, '"+pColumn.source+"', 'kd_"+pColumn.source+"' ,'', _fTableGetInput(this), _fTableGetNext(this));")
...
//			lCell.setAttribute("align","center")
	...
	function _fTableWriteValue(pCell,pCellData){
		...
		pCell.align="left"
		eval("lArrTable=arrField"+lTable.id)
		if (lArrTable[pCell.cellIndex].is_readonly=="t"){
			...
			if (lInput) {
				switch (lArrTable[pCellData.cellIndex].type) {
                	case "numeric":
                        pCell.innerHTML=number_format(lInput.value,true);
                        pCell.align="right"
						lInput.value="";
	...
	function table (){
		...
		this.getIsi=function(){
			...
			for (lIndexRow=1;lIndexRow<this.table.rows.length-1;lIndexRow++){
				if (this.table.rows[lIndexRow].isEdit) lAlerttxt="Table baris ke "+lIndexRow+" masih dalam posisi edit.<br>"
			}
			if (lAlerttxt=="") {
				lStrIsi="";
				for (lIndexRow=1;lIndexRow<this.table.rows.length-1;lIndexRow++){
			...
			for (lIndex=0;lIndex<(lRowData.length-1);lIndex++){
				...
				for (var lIndex2=0;lIndex2<lCellData.length;lIndex2++){
					...
					if (this.columns[lIndex2].type=="checkbox") {
						...
						lCell.innerHTML=number_format(lCellData[lIndex2])
						lCell.align="right"
