...
<!-- content begin -->
			<table cellpadding="0" cellspacing="1" border="0" width="100%">
                <tr style="padding:0 5 0 5" bgcolor='#efefef'>
					...
					<td  style="padding:0 5 0 5"width="80%" colspan="3"><input type="hidden" name="code" value="<?=convert_html($code)?>" size="12" onKeyUp="fNextFocus(event,document.form1.is_laba_rugi)"><?=convert_html($code)?></td>
				</tr>
				<tr style="padding:0 5 0 5" bgcolor='#efefef'>
					<td style="padding:0 5 0 5"width="20%">Laba/Rugi Detail</td>
					<td  style="padding:0 5 0 5"width="30%" colspan="3"><input type="checkbox" name="is_laba_rugi" class="groove_text" value="t" <?=(($is_laba_rugi=="t")?"checked disabled":"")?> disabled onKeyUp="fNextFocus(event,document.form1.is_neraca)"></td>
				</tr>
				<tr style="padding:0 5 0 5" bgcolor='#efefef'>
					<td style="padding:0 5 0 5"width="20%">Neraca Detail</td>
					<td  style="padding:0 5 0 5"width="30%" colspan="3">
					<input type="checkbox" name="is_neraca" class="groove_text" value="t" <?=(($is_neraca=="t")?"checked disabled":"")?> disabled onKeyUp="fNextFocus(event,document.form1.description)"></td>
				</tr>
				<tr style="padding:0 5 0 5" bgcolor='#efefef'>
					<td style="padding:0 5 0 5"width="20%">Description</td>
					<td  style="padding:0 5 0 5"width="30%" colspan="3"><input type="hidden" name="description" class="groove_text" size="120" value="<?=convert_html($description)?>" onKeyUp="fNextFocus(event,document.form1.type_saldo)"><?=convert_html($description)?></td>
				</tr>
				<tr style="padding:0 5 0 5" bgcolor='#efefef'>
					<td style="padding:0 5 0 5"width="20%" class="fontColor">Saldo Type</td>
					<td  style="padding:0 5 0 5"width="30%" colspan="3"><select name="type_saldo" class="groove_text" onKeyUp="fNextFocus(event,document.form1.btnsimpan)" disabled>
                ...
                <tr bgcolor='#aaafff'><td height='1' colspan="4"></td></tr>
            </table>
            <table width="100%" cellpadding="0" cellspacing="1">
                <tr class="header" bgcolor="c8c8c8">
                    <td width="50%">COA</td>
                    <td>Description</td>
                </tr>
                <? create_list_coa_by_head_account($id_edit)?>
            </table>
		</td>
	</tr>
	<tr height="20"><td height="25" align="center" bgcolor="#D0E4FF" class="border">&nbsp;</td>
	</tr>
</table>
</form>
</body>
...
function cek_error(){
	...
	if($id_edit==""){
		...
		}elseif(strlen($code)!=2 || check_type(integer,$code)){
			$strmsg.="Code Harus 2 Digit.<br>";
			if(!$j_action) $j_action="document.form1.code.focus()";
...
function create_list_coa_by_head_account($p_head_account){
	$lrs = pg_query("select * from tblcoa where fk_head_account = '".$p_head_account."' order by fk_head_account,coa");
	while($lrow = pg_fetch_array($lrs)){
?>
		<tr bgcolor='#efefef'>
        	<td style="padding:0 5 0 5"><?=do_link($lrow['coa'],'chart_of_account_detail')?></td>
        	<td style="padding:0 5 0 5"><?=convert_html($lrow['description']." ".$lrow['additional_description'])?></td>
        </tr>
<?
	}
}
