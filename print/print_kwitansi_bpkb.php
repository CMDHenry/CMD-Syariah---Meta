...
// Create a PDF object and set up the properties
		...
		$query="
			select tblcabang.alamat as alamat_cabang,* from (select * from  data_fa.tblpembayaran_bpkb where no_kwitansi='".$id_edit."') as tblpembayaran
		left join (
		...
		$total_pembayaran=$lrow["nilai_bayar"];
		...
		$pdf->Text($x4,$y, 'Pengambilan BPKB');
		$pdf->Text($x5,$y, '');
		$pdf->Text($x6,$y, '');
