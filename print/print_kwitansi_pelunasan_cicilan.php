...
// Create a PDF object and set up the properties
		...
		$query="
			select tblcabang.alamat as alamat_cabang,* from (select * from  data_fa.tblpelunasan_cicilan where no_kwitansi='".$id_edit."') as tblpembayaran_cicilan
		left join (
		...
		$pdf->Text($x4,$y, 'Sisa Pokok ');
		$pdf->Text($x5,$y, ':');
		$pdf->setY($y-10);
		$pdf->Cell(50,15,''.convert_money("",$lrow["sisa_pokok"]).'',0,'','R','','',$x5);
		//$pdf->Text($x6,$y, ''.convert_money("",$lrow["sisa_pokok"]));
		...
		$pdf->Text($x5,$y, ':');
		$pdf->setY($y-10);
		$pdf->Cell(50,15,''.convert_money("",$lrow["bunga_berjalan"]).'',0,'','R','','',$x5);				
		...
		$pdf->Text($x4,$y, "Pinalti ");
		$pdf->Text($x5,$y, ':');
		$pdf->setY($y-10);
		$pdf->Cell(50,15,''.convert_money("",$lrow["pinalti"]).'',0,'','R','','',$x5);				
							
		$pdf->Text($x10,$y, $lrow["nm_cabang"].', '.$tgl_bayar);		
		$y+=12;

		$pdf->Text($x4,$y, 'Sisa Angsuran ');
		$pdf->Text($x5,$y, ':');
		$pdf->setY($y-10);
		$pdf->Cell(50,15,''.convert_money("",$lrow["sisa_angsuran"]).'',0,'','R','','',$x5);	
		...
		$pdf->Text($x5,$y, ':');
		$pdf->setY($y-10);
		$pdf->Cell(50,15,''.convert_money("",$lrow["nilai_bayar_denda"]).'',0,'','R','','',$x5);	
		$y+=12;
		
		$pdf->Text($x4,$y, "Ta'zir");
		$pdf->Text($x5,$y, ':');
		$pdf->setY($y-10);
		$pdf->Cell(50,15,''.convert_money("",$lrow["nilai_bayar_denda2"]).'',0,'','R','','',$x5);	
		$y+=12;
